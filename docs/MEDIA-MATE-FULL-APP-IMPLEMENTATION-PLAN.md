...
[`MEDIA-MATE-PRODUCT-DIRECTION.md`](./MEDIA-MATE-PRODUCT-DIRECTION.md). The
...
3. **The product remains local-first and account-free.** No cloud service,
   remote API, or user identity is required to use Media-mate.
...
1. The user selects a detected volume or folder and identifies it as a camera
   source; Media-mate records volume/source metadata and scans without writing.
...
4. Media-mate copies each file into an incomplete destination, records
...
2. Media-mate inventories it, captures provenance, and proposes asset/clip
...
- Development runs `python -m media_mate.service`; packaged builds launch a
...
- Avoid automatic system mutation: Media-mate may diagnose missing tools and
