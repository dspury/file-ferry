...
Layout:
    ferry probe <path>
    ferry organize <path> --root <root>
    ferry proxy <path> --out <dir>
    ferry resolve create <path> --project <name> [...]
    ferry verify <path>
    ferry log
    ferry run <path> [--organize --proxy --resolve-project --verify]
"""
...
from ferry.models import FerryConfig, ResolveProjectSpec
from ferry.organize import organize_path
from ferry.probe import probe_path
from ferry.proxy import generate_proxies
from ferry.verify import verify_folder

DEFAULT_DB_PATH = Path.home() / ".ferry" / "ferry.db"


def _load_create_resolve_project() -> Any:
    """Import the Resolve adapter on demand.

    DaVinci Resolve is an optional integration: it has no pip dependency and is
    detected at runtime, so nothing in the CLI import graph should reach for it
    until a Resolve command actually runs. The TUI lazy-imports the same way.
    """
    try:
        from ferry.resolve import create_resolve_project
    except ImportError as e:  # pragma: no cover - in-package import
        raise click.ClickException(
            f"the DaVinci Resolve integration could not be loaded ({e}). "
            "Resolve support is optional; every other command works without it."
        ) from e
    return create_resolve_project
...
@click.option(
    ...
    default=None,
    envvar="FERRY_CONFIG",
    help="Path to ferry.toml config file.",
)
@click.option(
    "--no-tui",
    is_flag=True,
...
def main(ctx: click.Context, db: Path, config_path: Path | None, no_tui: bool) -> None:
    ...
    if ctx.invoked_subcommand is None:
        if no_tui:
            ...
            from ferry.tui import main as tui_main
...
# untouched.
from ferry.cli_vnext import (  # noqa: E402  (after group def)
    intake_group,
    jobs_group,
    project_group,
    receipt_group,
...
def _get_config(ctx: click.Context) -> FerryConfig:
...
@main.group()
def resolve() -> None:
    """DaVinci Resolve project commands (optional integration).

    Requires DaVinci Resolve to be installed separately — it is detected at
    runtime and is not a pip dependency. Without it, a JSON manifest is
    written instead of a .drp project.
    """
...
def resolve_create(
    ...
    create_resolve_project = _load_create_resolve_project()
...
@click.option(
    ...
    "do_resolve",
    is_flag=True,
    help="Resolve project creation step. Requires DaVinci Resolve installed "
    "separately; writes a JSON manifest instead when it is unavailable.",
)
