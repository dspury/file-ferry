...
Files without probe data in the audit log are skipped (run `ferry probe`
...
def organize_path(
    ...
    command = f"ferry organize {source} --root {dest_root}"
    if do_move:
        command += " --move"
    ...
    for f in files:
        try:
            ...
            if probe is None:
                files_skipped += 1
                errors.append(f"{f.name}: no probe data — run `ferry probe` first")
                continue
