...
Migrations are modules under :mod:`ferry.persistence.migrations`,
...
from ferry.persistence import backup
from ferry.persistence.connection import database_exists, open_connection
from ferry.persistence.schema_meta import (
    current_version,
    ensure_schema_meta,
    set_version,
)
...
def discover_migrations(pkg: object = None) -> list[Migration]:
    ...
    The default package is :mod:`ferry.persistence.migrations`;
    ...
    """
    if pkg is None:
        pkg = importlib.import_module("ferry.persistence.migrations")
    out: list[Migration] = []
