"""Keyboard-first Textual interface for ferry."""
...
from ferry.models import ChecksumAlgo, FerryConfig, OrganizeConfig
from ferry.organize import compute_output_tree
from ferry.probe import SYSTEM_ARTIFACT_NAMES
...
class HomeScreen(Screen[Any]):
    ...
    def on_mount(self) -> None:
        self.query_one("#hero", Container).border_title = "ferry"
        self._refresh_stats()
...
class PipelineScreen(Screen[Any]):
            ...
            with Vertical(id="run-pane"):
                ...
                with Vertical(id="config-panel"):
                    with Horizontal(id="steps"):
                        ...
                        resolve_step = Checkbox("4 Resolve", id="resolve")
                        resolve_step.tooltip = (
                            "Optional — requires DaVinci Resolve installed separately. "
                            "Without it this step writes a JSON manifest instead."
                        )
                        yield resolve_step
    ...
    def _run_resolve_step(self, ctx: _PipelineItemContext) -> str:
        from ferry.models import ResolveProjectSpec
...
class SettingsScreen(Screen[Any]):
    ...
    def action_save(self) -> None:
        ...
        cfg = FerryConfig.model_validate(
            {
                **old.model_dump(),
