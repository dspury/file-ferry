"""Configuration loader and persistence for ferry.

Loads a ferry.toml file into a FerryConfig pydantic model. When no
config file is found, returns FerryConfig() (all defaults).
...
def load_config(path: Path | None = None) -> FerryConfig:
    ...
    if path is not None:
        ...
        candidates.append(Path.cwd() / "ferry.toml")
        home_config = Path.home() / ".ferry" / "config.toml"
...
def config_target(explicit: Path | None) -> Path:
    if explicit:
        return explicit
    local = Path.cwd() / "ferry.toml"
    return local if local.exists() else Path.home() / ".ferry" / "config.toml"


def save_config(config: FerryConfig, path: Path) -> None:
