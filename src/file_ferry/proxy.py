...
from file_ferry.probe import _format_errors, is_system_artifact
...
def find_ffmpeg(config: FerryConfig | None = None) -> str:
...
def _probe_output_metadata(output: Path) -> tuple[int, int, float]:
    ...
    """
    try:
        from file_ferry.probe import find_ffprobe, probe_file
...
def generate_proxies(
    ...
    from file_ferry.probe import ProbeError, find_ffprobe, probe_file

    ffprobe_path = find_ffprobe(cfg)
    command = f"ferry proxy {source} --out {output_dir}"
