...
from ferry.cli import _fetch_recent_runs, main
...
class TestTopLevel:
    ...
    def test_no_args_launches_tui(self, runner: CliRunner) -> None:
        with patch("ferry.tui.main") as mock_tui:
...
class TestProbeCommand:
    def test_probe_uses_loaded_config(
        ...
        with patch("ferry.cli.probe_path", return_value=[]) as mock_probe:
    ...
    def test_probe_with_files(self, runner: CliRunner, tmp_path: Path, tmp_db: Path) -> None:
        ...
        with (
            patch("ferry.cli.probe_path") as mock_probe,
        ):
...
class TestOrganizeCommand:
    def test_organize(self, runner: CliRunner, tmp_path: Path, tmp_db: Path) -> None:
        ...
        with patch("ferry.cli.organize_path") as mock_org:
            from ferry.models import OrganizeResult
...
class TestProxyCommand:
    def test_proxy(self, runner: CliRunner, tmp_path: Path, tmp_db: Path) -> None:
        ...
        with patch("ferry.cli.generate_proxies") as mock_gen:
            from ferry.models import ProxyBatchResult
...
class TestResolveCommand:
    def test_resolve_create_when_resolve_available(
        ...
        with patch("ferry.resolve.create_resolve_project") as mock_create:
            from datetime import datetime

            from ferry.models import ResolveProjectResult
...
class TestVerifyCommand:
    def test_verify_uses_loaded_config(
        ...
        with patch("ferry.cli.verify_folder") as mock_verify:
            from ferry.models import ChecksumAlgo, VerificationReport
    ...
    def test_verify_clean(self, runner: CliRunner, tmp_path: Path, tmp_db: Path) -> None:
        ...
        with patch("ferry.cli.verify_folder") as mock_v:
            from datetime import datetime
...
class TestRunCommand:
    def test_run_probe_only(self, runner: CliRunner, tmp_path: Path, tmp_db: Path) -> None:
        ...
        with patch("ferry.cli.probe_path") as mock_probe:
    ...
    def test_run_with_all_steps(self, runner: CliRunner, tmp_path: Path, tmp_db: Path) -> None:
        ...
        with (
            patch("ferry.cli.probe_path") as mock_probe,
            patch("ferry.cli.organize_path") as mock_org,
            patch("ferry.cli.generate_proxies") as mock_proxy,
            patch("ferry.resolve.create_resolve_project") as mock_resolve,
            patch("ferry.cli.verify_folder") as mock_verify,
        ):
            from datetime import datetime
